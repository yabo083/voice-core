...
| Component | Version | Licence | Why it is in the tree |
|---|---|---|---|
| Irodori-TTS (engine source) | v4 codebase for v4.1-Small | **MIT** — `voice-core/tts/irodori-tts/webui/Irodori-TTS/LICENSE`, "Copyright (c) 2026 Aratako"; `pyproject.toml:6` `license = "MIT"` | The synthesis implementation. `package.ps1:198` copies `webui/` to `runtime/engine/webui/`. MIT permits this redistribution with the copyright and permission notice retained, and MIT is GPL-3.0-compatible. The installed tree comes from the `yabo083/Irodori-TTS` fork (branch `voice-core`) of the upstream `Aratako/Irodori-TTS`: same file layout and licence, plus two latency patches (conditional-encoding dedup, CUDA Graph replay) made by this project's maintainer. `bootstrap.ps1` clones the fork; `FORK.md` inside the clone documents that its `origin` remote stays pristine upstream |
...
`app/VoiceCoreTray/assets/*.ico` and `manager/src-tauri/icons/*` are the application mark,
authored for this project. It is not derived from any game or third-party asset and is
covered by this project's own licence.
