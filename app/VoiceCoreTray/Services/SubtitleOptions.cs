...
///   --presenter                  subtitle surface only: no tray icon, no context menu, no
///                                status window, and never a runtime
///
...
public sealed class SubtitleOptions
{
    ...
    /// <summary>
    /// Subtitle surface only (--presenter): the role <c>VoiceCore.exe</c> spawns. No tray
    /// icon, no context menu, no status window. The dialog, the global hotkeys, the wheel
    /// gesture and the SSE subscription are unaffected — they are the job.
    /// </summary>
    public bool PresenterMode { get; init; }

    private bool _noRuntimeFlag;

    /// <summary>Never launch a runtime (--no-runtime, implied by --presenter).</summary>
    // Computed, not stored: a presenter that starts its own runtime is unrepresentable.
    // VoiceCore.exe has already started one and owns its lifetime, and a second would lose
    // the race for 127.0.0.1:8760 and then be nobody's child.
    public bool NoRuntime
    {
        get => _noRuntimeFlag || PresenterMode;
        init => _noRuntimeFlag = value;
    }
