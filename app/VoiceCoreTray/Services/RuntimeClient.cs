...
public sealed class RuntimeClient : IAsyncDisposable
{
    ...
    /// dist layout: &lt;root&gt;\bin\presenter\VoiceCorePresenter.exe with &lt;root&gt;\data;
    /// dev layout: the repo's own data dir beside Cargo.toml. The walk below is what makes
    /// the move from bin\app\ to bin\presenter\ a non-event: it matches on a SIBLING marker
    /// (bin\voice-core-runtime.exe), never on this executable's own directory name.
    ...
    private async Task<string> RefusedDetailAsync(int code)
    {
        ...
        return live
            ? $"HTTP {code}：runtime 在运行但拒绝了令牌，说明它和本程序解析到了不同的数据目录；核对 {token}"
