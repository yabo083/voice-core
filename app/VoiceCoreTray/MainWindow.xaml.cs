...
///
/// TWO ROLES since 1.2.0, selected by <c>--presenter</c>
/// (<see cref="SubtitleOptions.PresenterMode"/>):
///
/// * default — the standalone tray a developer launches: tray icon, context menu, status
///   view, and it starts and stops the runtime itself. Slated for deletion once
///   <c>VoiceCore.exe</c> has shipped; kept for one release so the presenter can still be
///   exercised without the GUI.
/// * <c>--presenter</c> — the subtitle surface <c>VoiceCore.exe</c> spawns. Same dialog,
///   same hotkeys, same wheel gesture, same SSE subscription; no shell icon, no menu, no
///   window, and it never starts or stops a runtime. Only the input pump and the dialog
///   need an HWND, so the window is still created — it is simply never shown.
/// </summary>
public sealed partial class MainWindow : Window
{
    ...
    public MainWindow(SubtitleOptions options)
    {
        ...
        _options = options;
        _presenterMode = options.PresenterMode;

        Title = "voice-core";

        if (_presenterMode)
        {
            // Two steps, and both are load-bearing (H.NotifyIcon 2.3.0, TaskbarIcon.cs /
            // TaskbarIcon.Dispose.cs):
            //   Remove  — the shell icon is added from the element's Loaded handler, and Loaded
            //             fires off the first layout pass even for a window nothing ever shows,
            //             which is how the tray icon appears today with no call to Activate().
            //             Detaching it here, synchronously, is what makes "no tray icon" true,
            //             and it takes the context menu with it because the menu is this icon's
            //             ContextFlyout and has no other trigger.
            //   Dispose — the ctor subscribes TrayIcon.MessageWindow.TaskbarCreated, whose
            //             handler calls TrayIcon.Create() unconditionally. Without Dispose, an
            //             Explorer restart would resurrect an icon that was never in the tree.
            //             Dispose tears down that message window; it is idempotent, and on
            //             WinUI nothing re-disposes at process exit.
            Root.Children.Remove(TrayIcon);
            TrayIcon.Dispose();
        }
        else
        {
        ...
        if (!_presenterMode)
        {
            // The tooltip and the status view are the only readers of GET /api/status, and a
            // presenter has neither. Skipping the timer removes a 5 s poll per presenter that
            // VoiceCore.exe is already making for the state it actually renders.
            _ = RefreshTrayAsync();
        ...
        // Never true under --presenter: SubtitleOptions.NoRuntime is computed from
        // (--no-runtime || --presenter), so this is the "no runtime launch" guarantee.
    ...
    /// <summary>The notification area gets the flat monochrome mark (slot C of the design
    /// contract), not the app tile: it sits in a row of system glyphs at 16 px, where a tile
    /// reads as a foreign object. icon.ico stays the window and installer icon.
    /// Slot C is #dadada throughout, so it assumes the dark taskbar the rest of the product
    /// is designed against.</summary>
    ...
    {
        _note = note;
        // A presenter has no visible surface at all, so a hotkey conflict or a runtime error
        // would otherwise be reported to a window nobody can open. The boot log is the only
        // place left that a human can read afterwards.
        if (_presenterMode)
        {
            App.Log("note: " + note);
            return;
    ...
    {
        // Nothing to refresh: the icon was detached in the constructor and the status view
        // can never be shown, so this would be a round trip whose result is discarded.
        if (_presenterMode) return;
    ...
    /// <summary>
    /// 服务状态 toggles, because nothing else can dismiss this window:
    /// <c>SetBorderAndTitleBar(hasBorder: false, hasTitleBar: false)</c> leaves the frame no
    /// close affordance, and it is always-on-top and hidden from Alt+Tab. Hide, never Close —
    /// this HWND owns the <c>RegisterHotKey</c> registrations
    /// (<see cref="HotkeyManager"/>) and the thread that pumps the
    ...
    {
        // The ONLY place this window is ever made visible (AppWindow.Show + Activate below).
        // Under --presenter there is no menu that could reach it, but the guard is what makes
        // "no status window" a property of the code rather than of the missing menu.
        if (_presenterMode) return;
    ...
    {
        // Starting the backend belongs to VoiceCore.exe. Unreachable under --presenter (this
        // is a detached menu item's handler), and refused anyway.
        if (_presenterMode) return;
    ...
    {
        // StopAsync call site 1 of 2. A presenter that could stop the runtime would take the
        // backend down under VoiceCore.exe and every other subscriber; killing it is the GUI's
        // Quit, and only the GUI's Quit.
        if (_presenterMode) return;
    ...
    private async void OnExitClick(object sender, RoutedEventArgs e)
    {
        ...
        if (!_presenterMode)
        {
            // StopAsync call site 2 of 2, and the tray icon this branch owns. Exiting a
            // presenter must leave the runtime running: the GUI spawned both and stops them in
            // its own order.
            await _runtime.StopAsync();
            TrayIcon.Dispose();
        }
