...
# What the panel accepts too (`config_edit::import_avatar`): a portrait is decoded by the
# subtitle dialog, so the set is what a WinUI Image can load, not everything Pillow can.
AVATAR_SUFFIXES = {".png", ".jpg", ".jpeg", ".webp", ".bmp"}
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Portrait image. Copied into the pack, so the face travels with the voice.",
    )
...
def install(args: argparse.Namespace) -> None:
    ...
    # Two files, and each says only what it owns (docs/voicepack-spec.md). The manifest goes
    # inside the pack and describes the voice; the config entry is a pointer, because it is
    # generated and must not outrank what the pack says about itself.
    manifest: dict = {"schema": 1, "id": args.id, "name": args.name or args.id}
    if args.character:
        manifest["character"] = args.character
    manifest["engine"] = args.engine
    manifest["kind"] = kind
    manifest["languages"] = list(args.languages)

    avatar = args.avatar.expanduser().resolve() if args.avatar else None
    if avatar is not None:
        if not avatar.is_file():
            raise SystemExit(f"--avatar does not exist: {avatar}")
        if avatar.suffix.lower() not in AVATAR_SUFFIXES:
            raise SystemExit(
                f"--avatar must be one of {', '.join(sorted(AVATAR_SUFFIXES))}; got {avatar.suffix}"
            )
        # Relative to the pack, which is what makes the pack movable.
        manifest["avatar"] = f"avatar{avatar.suffix.lower()}"
    ...
    entry: dict = {
        "id": args.id,
        "path": (
            f"voicepacks/{args.id}"
            if kind == "lora-adapter"
            else f"voicepacks/{args.id}/{pack.name}"
        ),
    }
    ...
    if avatar is not None:
        print(f"avatar     {avatar}   -> {manifest['avatar']} (inside the pack)")
    print("voicepack.json (inside the pack):")
    for line in json.dumps(manifest, ensure_ascii=False, indent=2).splitlines():
        print(f"  {line}")
    print("config.json entry (a pointer):")
    ...
    if avatar is not None:
        shutil.copy2(avatar, target / manifest["avatar"])
    # A directory pack keeps its manifest inside; a single-file pack takes a sidecar named
    # after it, which is where the runtime looks (`Manifest::beside`).
    manifest_file = (
        target / "voicepack.json"
        if kind == "lora-adapter"
        else target / f"{pack.stem}.voicepack.json"
    )
