...
fn load_runtime_file(data_dir: &Path) -> Result<RuntimeFile> {
    ...
    // JSONC, like `config.json` beside it: both are files the error messages tell a human
    // to go and edit, and a `//` note explaining why a path was overridden must not turn
    // into "key must be a string at line 2".
    //
    ...
    serde_json::from_str(&voice_core::jsonc::to_json(&raw))
